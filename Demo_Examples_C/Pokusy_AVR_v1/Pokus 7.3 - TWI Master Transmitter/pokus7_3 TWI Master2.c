  ...
  {         
    ...
    if ((TWSR & 0xF8) == 0x08)  // start bol odoslany
    {
      ...
      {        
        ...
        {
          cislica(5); //delay_ms(1000);
        }                                    
      }
      
      if ((TWSR & 0xF8) == 0x20)  // SLA+W poslany, NOT ACK prijaty                 
      {
        cislica(6); //delay_ms(1000);
      }                                                                                                   
    
      TWCR = (1 << TWINT) | (1 << TWSTA) | (1 << TWSTO) | (1 << TWEN);  // posle stop bit a novy start bit      
    }    
    //delay_ms(500);
  }
}
